        ...
        <div class="expanded row margin-top-20">
			<form method="post" action="<?php echo url('prestacao/public/CadNota/create');?>" id="nota">
				...
				<div class="columns large-12">        
					...
					<input type="radio" name="escolha" value="automatico" id="aut"><label for="aut" readonly>Automatico</label></input>
					<input type="radio" name="escolha" value="manual" id="man"><label for="man" checked>Manual</label></input>
				</div> 
				...
				<div class="columns large-4">
					...
					<select  id="fornecedor" name="fornecedor"  required  <?php if(isset($dados['fornecedor'])){echo "value='".$dados['fornecedor']."'";} ?>>
						<?php
							$valorFor= isset($dados['fornecedor']) ? $dados['fornecedor'] : null;
	...
	{	
		...
		data: {search: "", pagination:<?php echo "'".$valorFor."'"; ?>  },
		success: function (response)
		{
			$('#fornecedor').html(response);
		}
		});
	}
